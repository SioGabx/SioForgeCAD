...
namespace SioForgeCAD
{
    public class Commands
    {
        ...
        [CommandMethod("CreateInternalDrawing")]
        public static void CreateInternalDrawing()
        {
            ...
            {
                // Créez un nouveau dessin en mémoire
                Database inMemoryDb = new Database(true, true);
                using (DocumentLock docLock = doc.LockDocument())
                {
                    // Ajoutez le nouveau dessin à la liste des documents actifs
                    DocumentCollection docCol = Application.DocumentManager;

                    // Ouvrez l'espace modèle du nouveau dessin pour modification
                    using (Transaction inMemoryTransaction = inMemoryDb.TransactionManager.StartTransaction())
                    {
                        BlockTableRecord modelSpace = inMemoryTransaction.GetObject(SymbolUtilityServices.GetBlockModelSpaceId(inMemoryDb), OpenMode.ForWrite) as BlockTableRecord;

                        // Ajoutez une ligne à l'espace modèle du nouveau dessin
                        Line sampleLine = new Line(new Point3d(0, 0, 0), new Point3d(10, 10, 0));
                        modelSpace.AppendEntity(sampleLine);
                        inMemoryTransaction.AddNewlyCreatedDBObject(sampleLine, true);

                        // Terminez la transaction pour l'espace modèle
                        inMemoryTransaction.Commit();
                    }
                }

                // Terminez la transaction principale
                transaction.Commit();
            }
        }
        ...
        //[CommandMethod("CopyAndReplaceObject")]
        //public static void CopyAndReplaceObject()
        //{
        //    // Obtenez le document actif
        //    Document doc = Application.DocumentManager.MdiActiveDocument;
        //    var db = doc.Database;
        //    // Obtenez l'éditeur de texte
        //    Editor editor = doc.Editor;

        //    // Sélectionnez un objet dans le dessin
        //    PromptEntityResult result = editor.GetEntity("Sélectionnez un objet à copier : ");
        //    if (result.Status != PromptStatus.OK)
        //    {
        //        editor.WriteMessage("Sélection annulée ou invalide.");
        //        return;
        //    }

        //    ObjectId selectedObjectId = result.ObjectId;

        //    // Commencez une transaction pour le dessin en mémoire

        //    BlockReference selectedEntity;
        //    using (Transaction originalTransaction = doc.TransactionManager.StartTransaction())
        //    {
        //        selectedEntity = (originalTransaction.GetObject(selectedObjectId, OpenMode.ForWrite) ).Clone() as BlockReference;

        //    }
        //    Database inMemoryDb = new Database(true, true);
        //    using (Transaction memoryTransaction = inMemoryDb.TransactionManager.StartTransaction())
        //    {
        //        // Ouvrez l'espace modèle du dessin en mémoire pour modification
        //        BlockTableRecord memoryModelSpace = memoryTransaction.GetObject(SymbolUtilityServices.GetBlockModelSpaceId(inMemoryDb), OpenMode.ForWrite) as BlockTableRecord;

        //        // Copiez l'objet sélectionné dans le dessin en mémoire

        //        if (selectedEntity != null)
        //        {
        //            memoryModelSpace.AppendEntity(selectedEntity);
        //            memoryTransaction.AddNewlyCreatedDBObject(selectedEntity, true);

        //            // Renommez le bloc en "tesryh"
        //            if (selectedEntity is BlockReference)
        //            {
        //                BlockReference blockRef = selectedEntity as BlockReference;
        //                BlockTableRecord blockRecord = memoryTransaction.GetObject(selectedEntity.BlockTableRecord, OpenMode.ForWrite) as BlockTableRecord;

        //                // Renommez le bloc
        //                blockRecord.Name = "capucine";
        //            }



        //            // Terminez la transaction pour le dessin en mémoire
        //            memoryTransaction.Commit();

        //            // Commencez une transaction pour le dessin d'origine

        //            // Ouvrez l'espace modèle du dessin d'origine pour modification


        //            // Terminez la transaction pour le dessin d'origine
        ...
        //    using (Transaction originalTransaction = doc.TransactionManager.StartTransaction())
        //    {
        //        Generic.Erase(selectedEntity.ObjectId);
        //        BlockTable acBlkTbl = originalTransaction.GetObject(db.BlockTableId, OpenMode.ForRead) as BlockTable;
        //        BlockTableRecord acBlkTblRec = originalTransaction.GetObject(acBlkTbl[BlockTableRecord.ModelSpace], OpenMode.ForWrite) as BlockTableRecord;
        //        acBlkTblRec.AppendEntity(selectedEntity);
        //        originalTransaction.AddNewlyCreatedDBObject(selectedEntity, true);
        ...
        //    ObjectId selectedObjectId = result.ObjectId;
        //    Database inMemoryDb = new Database(true, true);
        //        db.Clone() as Database;
        //    // Commencez une transaction pour le dessin d'origine
        //    using (Transaction originalTransaction = db.TransactionManager.StartTransaction())
        //    {

        //        using (Transaction memoryTransaction = inMemoryDb.TransactionManager.StartTransaction())
        //        {
        //            // Copiez l'objet sélectionné dans le dessin d'origine
        //            BlockReference selectedEntity = originalTransaction.GetObject(selectedObjectId, OpenMode.ForRead) as BlockReference;
        //            // Copiez l'objet dans le dessin en mémoire

        //            ObjectId blockId = selectedEntity.BlockTableRecord;
        //            BlockTableRecord blockRecordInOriginalDoc = originalTransaction.GetObject(blockId, OpenMode.ForRead) as BlockTableRecord;
        //            blockRecordInOriginalDoc.Name = "test";
        //            inMemoryDb.SaveAs(@"C:\Users\franc\Downloads\Nouveau dossier (14)\test.dwg", DwgVersion.Current);
        ...
        //    Entity copiedEntity = selectedEntity.Clone() as Entity;
        //    memoryModelSpace.AppendEntity(copiedEntity);
        //    memoryTransaction.AddNewlyCreatedDBObject(copiedEntity, true);

        //    // Obtenez le BlockTableRecord du BlockReference dans le document d'origine
        //    BlockReference blockRef = selectedEntity as BlockReference;
        //    ObjectId blockId = blockRef.BlockTableRecord;
        //    BlockTableRecord blockRecordInOriginalDoc = originalTransaction.GetObject(blockId, OpenMode.ForRead) as BlockTableRecord;

        //    // Copiez les objets du BlockTableRecord dans le document en mémoire
        //    foreach (ObjectId objectId in blockRecordInOriginalDoc)
        //    {
        //        Entity entityInOriginalDoc = originalTransaction.GetObject(objectId, OpenMode.ForRead) as Entity;
        //        if (entityInOriginalDoc != null)
        //        {
        //            Entity entityInMemoryDoc = entityInOriginalDoc.Clone() as Entity;
        //            memoryModelSpace.AppendEntity(entityInMemoryDoc);
        //            memoryTransaction.AddNewlyCreatedDBObject(entityInMemoryDoc, true);
        //        }
        //    }


        //    BlockTable bt = (BlockTable)memoryTransaction.GetObject(inMemoryDb.BlockTableId, OpenMode.ForRead); //Getting block table to put new record in
        //    bt.UpgradeOpen();
        //    BlockTableRecord blockRecordInMemoryDoc = blockRecordInOriginalDoc.Clone() as BlockTableRecord;
        //    // Mettez à jour le BlockReference pour utiliser le nouveau BlockTableRecord
        //    blockId = bt.Add(blockRecordInMemoryDoc);

        //    blockRecordInMemoryDoc.Name = "capucine";
        //    (copiedEntity as BlockReference).BlockTableRecord = blockRecordInMemoryDoc.Id;
        ...
        //    BlockTable bt2 = (BlockTable)originalTransaction.GetObject(db.BlockTableId, OpenMode.ForRead); //Getting block table to put new record in
        //    bt2.UpgradeOpen();
        //    BlockTableRecord blockRecordInMemoryDocClone = blockRecordInMemoryDoc.Clone() as BlockTableRecord;
        //    // Mettez à jour le BlockReference pour utiliser le nouveau BlockTableRecord
        //    blockId = bt2.Add(blockRecordInMemoryDocClone);
        //    BlockTableRecord ModelSpace2 = originalTransaction.GetObject(SymbolUtilityServices.GetBlockModelSpaceId(db), OpenMode.ForWrite) as BlockTableRecord;
        //    var newBlkRef = copiedEntity.Clone() as BlockReference;
        //    (newBlkRef).BlockTableRecord = blockRecordInMemoryDocClone.Id;
        //    ModelSpace2.AppendEntity(newBlkRef);
        //    originalTransaction.AddNewlyCreatedDBObject(newBlkRef, true);
        //    originalTransaction.Commit();
        ...
        [CommandMethod("CopyObjectsBetweenDatabases")]
        public static void CopyObjectsBetweenDatabases()
        {
            ...
            acObjIdColl.Add(result.ObjectId);
            var OldName = "";
            ...
            {
                OldName = (result.ObjectId.GetEntity() as BlockReference).GetBlockReferenceName();
            }


            Database acDbNewDoc = new Database(true, true);
            ...
            using (Transaction acTrans = acDbNewDoc.TransactionManager.StartTransaction())
            {
                ...
                acIdMap = new IdMapping();
                acCurDb.WblockCloneObjects(acObjIdColl, acBlkTblRecNewDoc.ObjectId, acIdMap, DuplicateRecordCloning.Ignore, false);
                BlockTableRecord btr = (BlockTableRecord)acTrans.GetObject(acBlkTblNewDoc[OldName], OpenMode.ForWrite);
                btr.Name = "prout";
                // Save the copied objects to the database
                acTrans.Commit();
            }

            ObjectId newBlocRefenceId = acIdMap[result.ObjectId].Value;


            acObjIdColl = new ObjectIdCollection();
            acObjIdColl.Add(newBlocRefenceId);
            // Start a transaction in the new database
            using (Transaction acTrans2 = acCurDb.TransactionManager.StartTransaction())
            {
                // Open the Block table for read
                BlockTable acBlkTblNewDoc2 = acTrans2.GetObject(db.BlockTableId, OpenMode.ForRead) as BlockTable;

                // Open the Block table record Model space for read
                BlockTableRecord acBlkTblRecNewDoc2 = acTrans2.GetObject(acBlkTblNewDoc2[BlockTableRecord.ModelSpace], OpenMode.ForWrite) as BlockTableRecord;
                
                // Clone the objects to the new database
                IdMapping acIdMap2 = new IdMapping();
                
                acCurDb.WblockCloneObjects(acObjIdColl, acBlkTblRecNewDoc2.ObjectId, acIdMap2, DuplicateRecordCloning.Ignore, false);
                acTrans2.Commit();
            }
            Generic.Erase(result.ObjectId);
            acDbNewDoc.SaveAs(@"C:\Users\franc\Downloads\Nouveau dossier (14)\test.dwg", DwgVersion.Current);
        }
